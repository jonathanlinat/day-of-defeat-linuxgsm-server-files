...
2.30b: Moved ALL config files to configs/h4c2r1ds_deaglesmapmanage/, if the directory exists otherwise configs/
...
{ 
   ...
   new banpath[64] 
   format(banpath,64,"%s/h4c2r1ds_deaglesmapmanage/mapstoban.ini",custompath) 
   new i=0 
   new line=0 
   new stextsize = 0 
   new linestr[256] 
...
change_custom_path(){
	...
	format(temp,64,"%s/h4c2r1ds_deaglesmapmanage/standardmaps.ini",custompath)
	if(!file_exists(temp)){
		format(temp,64,"%s/h4c2r1ds_deaglesmapmanage/mapchoice.ini",custompath)
		if(!file_exists(temp)){
			format(temp,64,"%s/h4c2r1ds_deaglesmapmanage/mapvault.dat",custompath)
			if(!file_exists(temp))
				dir_exists=false
		}
	}
