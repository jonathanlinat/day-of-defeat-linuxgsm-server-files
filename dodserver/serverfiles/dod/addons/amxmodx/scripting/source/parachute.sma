...
new const LOST_CHUTE_SOUND[] = "sound/misc/erriewind.wav"
new const PARA_MODELO[] = "models/parachute.mdl"
new const PARA_MODELW[] = "models/parachute_wings.mdl"
new const PARA_MODEL[] = "models/parachute_alternative.mdl"
...
public plugin_init()
{
    ...
    pEnabled    = register_cvar("amx_parachute", "1" )
    pFallSpeed  = register_cvar("amx_parachute_fallspeed", "100")
    pDetach     = register_cvar("amx_parachute_detach", "1")
    pAutoDeploy = register_cvar("amx_parachute_autorip", "650")
    pAutoRules  = register_cvar("amx_parachute_autoadmin", "2") //0|off 1|admin 2|all
    bind_pcvar_num(register_cvar("amx_parachute_safemode", "0"),g_UnBreakable)
    g_packHP    = register_cvar("amx_parachute_health", "50")
    g_debug     = register_cvar("amx_parachute_debug", "0")
    ...
    if (gCStrike)
    {
        ...
        pCost = register_cvar("amx_parachute_cost", "1000")
        pPayback = register_cvar("amx_parachute_payback", "75") //percentage
    ...
    register_forward(FM_PlayerPreThink, "amx_parachute_prethink", true)
}
...
if(is_user_connected(id))
{
    if( para_ent[id] )
    {
        ...
        if(bOF_run && bIsBot[id])
        {
            ...
            {
                set_task(2.0, "amx_parachute_reset", id)
            }
        }
        else
        {
